...
public class Resources {
    ...
    private void saveScoreEntries() throws IOException {
    	 Iterator<ScoreEntry> it = scoreEntries.iterator();
    	 
         String fileName="highscores.txt";
         try (PrintWriter fw = new PrintWriter(fileName)){
             while(it.hasNext()){
                 ScoreEntry score = it.next();
                 score.write(fw);
                }fw.close();
         } catch (IOException e) {
             e.printStackTrace();
    ...
    private void loadScoreEntries() {
    	 //scoreEntries-liste initialisieren 
        scoreEntries=new ArrayList<ScoreEntry>();
 
        //Highscores-tabelle aus dem Textdatei lesen
        String fileName="highscores.txt";
        //StringBuffer sb = new StringBuffer();
        try (BufferedReader br = new BufferedReader(new FileReader(fileName))){
            String line = null;
            //scoreEntries-liste füllen
            ...
            System.err.println("IOException: Allgemeiner Lesefehler mit " + fileName);
        }
 
        //scoreEntries-liste absteigend sortieren
     //   scoreEntries.sort(compareEntries);
