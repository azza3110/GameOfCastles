...
public class GameMap {
	...
	/**
	 * Diese Methode gibt die naehste 3 Buerge zurueck
	 * @param node 
	 * @return liste der positionen der 3 naeste Burge von list of nodes
	 */
	private ArrayList<Node<Castle>> findNearestCastles(Node<Castle> node) {
		
		List<Node<Castle>> nodes = new ArrayList <Node<Castle>>(castleGraph.getNodes());
		nodes.remove(node);
		ArrayList<Node<Castle>> nearest = new ArrayList<Node<Castle>> ();
		int iVonNearest=0;
		while(nearest.size()!=3) {
			double abstand=-1;
			for(int i=0;i<nodes.size();i++) {
				double tmp=Math.sqrt(
						Math.pow(node.getValue().getLocationOnMap().getX()-nodes.get(i).getValue().getLocationOnMap().getX(), 2)+
						Math.pow(node.getValue().getLocationOnMap().getY()-nodes.get(i).getValue().getLocationOnMap().getY(), 2)
						);
				
				if(abstand ==-1 || tmp < abstand) {
					abstand=tmp;
					iVonNearest=i;
				}
			}
			nearest.add(nodes.remove(iVonNearest));
		}
		return nearest;
	}
	...
	private void generateEdges() {
		for(Node<Castle> node: castleGraph.getNodes()) {
			for(int i=0;i<3;i++) 
			castleGraph.addEdge(node,findNearestCastles(node).get(i));}}
