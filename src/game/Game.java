...
public class Game {
		...
		for (int i = 0; i < Math.min(rollAttacker.length, rollDefender.length); i++) {
			if (rollAttackerSorted[i] > rollDefenderSorted[i]) {
				if (defender.hasJoker()) {
					gameInterface.onDefenderLost(defender);
					if (defender instanceof AI) {
						jokerUsed = true;
					} else
				...
				if (attacker.hasJoker()) {
					gameInterface.onAttackerLost(attacker);
					if(defender instanceof AI) {
						jokerUsed=true;
					}else AttackThread.sleep(3000);
					if (jokerUsed) {
						jokerUsed = false;
						stopAttack();
						break;
	...
	public void jokerToUse() {
		jokerUsed = true;
	}

	public Player getPlayerWithMostCastles() {
		Player pl = null;
		List<Player> l = new ArrayList<>();
		for (Player p : players) {
			if (pl == null)
				pl = p;
			if (p.getNumRegions(this) > pl.getNumRegions(this)) {
				l = new ArrayList<>();
				pl = p;
			} else if (p.getNumRegions(this) == pl.getNumRegions(this)) {
				l.add(p);
				l.add(pl);
			}
		}
